package org.itxtech.synapseapi.multiprotocol.protocol116210.protocol;

public class ItemStackRequestPacket116210 {
    public static final int ACTION_TAKE = 0;
    public static final int ACTION_PLACE = 1;
    public static final int ACTION_SWAP = 2;
    public static final int ACTION_DROP = 3;
    public static final int ACTION_DESTROY = 4;
    public static final int ACTION_CRAFTING_CONSUME_INPUT = 5;
    public static final int ACTION_CRAFTING_MARK_SECONDARY_RESULT_SLOT = 6;
    public static final int ACTION_LAB_TABLE_COMBINE = 7;
    public static final int ACTION_BEACON_PAYMENT = 8;
    /**
     * @since 1.16.210
     */
    public static final int ACTION_MINE_BLOCK = 9;
    public static final int ACTION_CRAFTING_RECIPE = 10;
    public static final int ACTION_CRAFTING_RECIPE_AUTO = 11;
    public static final int ACTION_CREATIVE_CREATE = 12;
    public static final int ACTION_CRAFTING_RECIPE_OPTIONAL = 13;
    public static final int ACTION_CRAFTING_NON_IMPLEMENTED_DEPRECATED_ASK_TY_LAING = 14;
    public static final int ACTION_CRAFTING_RESULTS_DEPRECATED_ASK_TY_LAING = 15;
}
