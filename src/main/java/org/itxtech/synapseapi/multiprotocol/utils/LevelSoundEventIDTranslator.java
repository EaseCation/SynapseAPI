package org.itxtech.synapseapi.multiprotocol.utils;

import cn.nukkit.network.protocol.LevelSoundEventPacket;
import org.itxtech.synapseapi.multiprotocol.AbstractProtocol;

import java.util.HashMap;
import java.util.Map;

public class LevelSoundEventIDTranslator {

	public final static Map<Integer, Integer> v12ToNewBook;
	public static final int SOUND_ITEM_USE_ON = 0;
	public static final int SOUND_HIT = 1;
	public static final int SOUND_STEP = 2;
	public static final int SOUND_FLY = 3;
	public static final int SOUND_JUMP = 4;
	public static final int SOUND_BREAK = 5;
	public static final int SOUND_PLACE = 6;
	public static final int SOUND_HEAVY_STEP = 7;
	public static final int SOUND_GALLOP = 8;
	public static final int SOUND_FALL = 9;
	public static final int SOUND_AMBIENT = 10;
	public static final int SOUND_AMBIENT_BABY = 11;
	public static final int SOUND_AMBIENT_IN_WATER = 12;
	public static final int SOUND_BREATHE = 13;
	public static final int SOUND_DEATH = 14;
	public static final int SOUND_DEATH_IN_WATER = 15;
	public static final int SOUND_DEATH_TO_ZOMBIE = 16;
	public static final int SOUND_HURT = 17;
	public static final int SOUND_HURT_IN_WATER = 18;
	public static final int SOUND_MAD = 19;
	public static final int SOUND_BOOST = 20;
	public static final int SOUND_BOW = 21;
	public static final int SOUND_SQUISH_BIG = 22;
	public static final int SOUND_SQUISH_SMALL = 23;
	public static final int SOUND_FALL_BIG = 24;
	public static final int SOUND_FALL_SMALL = 25;
	public static final int SOUND_SPLASH = 26;
	public static final int SOUND_FIZZ = 27;
	public static final int SOUND_FLAP = 28;
	public static final int SOUND_SWIM = 29;
	public static final int SOUND_DRINK = 30;
	public static final int SOUND_EAT = 31;
	public static final int SOUND_TAKEOFF = 32;
	public static final int SOUND_SHAKE = 33;
	public static final int SOUND_PLOP = 34;
	public static final int SOUND_LAND = 35;
	public static final int SOUND_SADDLE = 36;
	public static final int SOUND_ARMOR = 37;
	public static final int SOUND_MOB_ARMOR_STAND_PLACE = 38;
	public static final int SOUND_ADD_CHEST = 39;
	public static final int SOUND_THROW = 40;
	public static final int SOUND_ATTACK = 41;
	public static final int SOUND_ATTACK_NODAMAGE = 42;
	public static final int SOUND_ATTACK_STRONG = 43;
	public static final int SOUND_WARN = 44;
	public static final int SOUND_SHEAR = 45;
	public static final int SOUND_MILK = 46;
	public static final int SOUND_THUNDER = 47;
	public static final int SOUND_EXPLODE = 48;
	public static final int SOUND_FIRE = 49;
	public static final int SOUND_IGNITE = 50;
	public static final int SOUND_FUSE = 51;
	public static final int SOUND_STARE = 52;
	public static final int SOUND_SPAWN = 53;
	public static final int SOUND_SHOOT = 54;
	public static final int SOUND_BREAK_BLOCK = 55;
	public static final int SOUND_LAUNCH = 56;
	public static final int SOUND_BLAST = 57;
	public static final int SOUND_LARGE_BLAST = 58;
	public static final int SOUND_TWINKLE = 59;
	public static final int SOUND_REMEDY = 60;
	public static final int SOUND_UNFECT = 61;
	public static final int SOUND_LEVELUP = 62;
	public static final int SOUND_BOW_HIT = 63;
	public static final int SOUND_BULLET_HIT = 64;
	public static final int SOUND_EXTINGUISH_FIRE = 65;
	public static final int SOUND_ITEM_FIZZ = 66;
	public static final int SOUND_CHEST_OPEN = 67;
	public static final int SOUND_CHEST_CLOSED = 68;
	public static final int SOUND_SHULKERBOX_OPEN = 69;
	public static final int SOUND_SHULKERBOX_CLOSED = 70;
	public static final int SOUND_ENDERCHEST_OPEN = 71;
	public static final int SOUND_ENDERCHEST_CLOSED = 72;
	public static final int SOUND_POWER_ON = 73;
	public static final int SOUND_POWER_OFF = 74;
	public static final int SOUND_ATTACH = 75;
	public static final int SOUND_DETACH = 76;
	public static final int SOUND_DENY = 77;
	public static final int SOUND_TRIPOD = 78;
	public static final int SOUND_POP = 79;
	public static final int SOUND_DROP_SLOT = 80;
	public static final int SOUND_NOTE = 81;
	public static final int SOUND_THORNS = 82;
	public static final int SOUND_PISTON_IN = 83;
	public static final int SOUND_PISTON_OUT = 84;
	public static final int SOUND_PORTAL = 85;
	public static final int SOUND_WATER = 86;
	public static final int SOUND_LAVA_POP = 87;
	public static final int SOUND_LAVA = 88;
	public static final int SOUND_BURP = 89;
	public static final int SOUND_BUCKET_FILL_WATER = 90;
	public static final int SOUND_BUCKET_FILL_LAVA = 91;
	public static final int SOUND_BUCKET_EMPTY_WATER = 92;
	public static final int SOUND_BUCKET_EMPTY_LAVA = 93;
	public static final int SOUND_ARMOR_EQUIP_CHAIN = 94;
	public static final int SOUND_ARMOR_EQUIP_DIAMOND = 95;
	public static final int SOUND_ARMOR_EQUIP_GENERIC = 96;
	public static final int SOUND_ARMOR_EQUIP_GOLD = 97;
	public static final int SOUND_ARMOR_EQUIP_IRON = 98;
	public static final int SOUND_ARMOR_EQUIP_LEATHER = 99;
	public static final int SOUND_ARMOR_EQUIP_ELYTRA = 100;
	public static final int SOUND_RECORD_13 = 101;
	public static final int SOUND_RECORD_CAT = 102;
	public static final int SOUND_RECORD_BLOCKS = 103;
	public static final int SOUND_RECORD_CHIRP = 104;
	public static final int SOUND_RECORD_FAR = 105;
	public static final int SOUND_RECORD_MALL = 106;
	public static final int SOUND_RECORD_MELLOHI = 107;
	public static final int SOUND_RECORD_STAL = 108;
	public static final int SOUND_RECORD_STRAD = 109;
	public static final int SOUND_RECORD_WARD = 110;
	public static final int SOUND_RECORD_11 = 111;
	public static final int SOUND_RECORD_WAIT = 112;
	public static final int SOUND_GUARDIAN_FLOP = 114;
	public static final int SOUND_ELDERGUARDIAN_CURSE = 115;
	public static final int SOUND_MOB_WARNING = 116;
	public static final int SOUND_MOB_WARNING_BABY = 117;
	public static final int SOUND_TELEPORT = 118;
	public static final int SOUND_SHULKER_OPEN = 119;
	public static final int SOUND_SHULKER_CLOSE = 120;
	public static final int SOUND_HAGGLE = 121;
	public static final int SOUND_HAGGLE_YES = 122;
	public static final int SOUND_HAGGLE_NO = 123;
	public static final int SOUND_HAGGLE_IDLE = 124;
	public static final int SOUND_CHORUSGROW = 125;
	public static final int SOUND_CHORUSDEATH = 126;
	public static final int SOUND_GLASS = 127;
	public static final int SOUND_POTION_BREWED = 128;
	public static final int SOUND_CAST_SPELL = 129;
	public static final int SOUND_PREPARE_ATTACK = 130;
	public static final int SOUND_PREPARE_SUMMON = 131;
	public static final int SOUND_PREPARE_WOLOLO = 132;
	public static final int SOUND_FANG = 133;
	public static final int SOUND_CHARGE = 134;
	public static final int SOUND_CAMERA_TAKE_PICTURE = 135;
	public static final int SOUND_LEASHKNOT_PLACE = 136;
	public static final int SOUND_LEASHKNOT_BREAK = 137;
	public static final int SOUND_GROWL = 138;
	public static final int SOUND_WHINE = 139;
	public static final int SOUND_PANT = 140;
	public static final int SOUND_PURR = 141;
	public static final int SOUND_PURREOW = 142;
	public static final int SOUND_DEATH_MIN_VOLUME = 143;
	public static final int SOUND_DEATH_MID_VOLUME = 144;
	public static final int SOUND_IMITATE_BLAZE = 145;
	public static final int SOUND_IMITATE_CAVE_SPIDER = 146;
	public static final int SOUND_IMITATE_CREEPER = 147;
	public static final int SOUND_IMITATE_ELDER_GUARDIAN = 148;
	public static final int SOUND_IMITATE_ENDER_DRAGON = 149;
	public static final int SOUND_IMITATE_ENDERMAN = 150;
	public static final int SOUND_IMITATE_EVOCATION_ILLAGER = 152;
	public static final int SOUND_IMITATE_GHAST = 153;
	public static final int SOUND_IMITATE_HUSK = 154;
	public static final int SOUND_IMITATE_ILLUSION_ILLAGER = 155;
	public static final int SOUND_IMITATE_MAGMA_CUBE = 156;
	public static final int SOUND_IMITATE_POLAR_BEAR = 157;
	public static final int SOUND_IMITATE_SHULKER = 158;
	public static final int SOUND_IMITATE_SILVERFISH = 159;
	public static final int SOUND_IMITATE_SKELETON = 160;
	public static final int SOUND_IMITATE_SLIME = 161;
	public static final int SOUND_IMITATE_SPIDER = 162;
	public static final int SOUND_IMITATE_STRAY = 163;
	public static final int SOUND_IMITATE_VEX = 164;
	public static final int SOUND_IMITATE_VINDICATION_ILLAGER = 165;
	public static final int SOUND_IMITATE_WITCH = 166;
	public static final int SOUND_IMITATE_WITHER = 167;
	public static final int SOUND_IMITATE_WITHER_SKELETON = 168;
	public static final int SOUND_IMITATE_WOLF = 169;
	public static final int SOUND_IMITATE_ZOMBIE = 170;
	public static final int SOUND_IMITATE_ZOMBIE_PIGMAN = 171;
	public static final int SOUND_IMITATE_ZOMBIE_VILLAGER = 172;
	public static final int SOUND_BLOCK_END_PORTAL_FRAME_FILL = 173;
	public static final int SOUND_BLOCK_END_PORTAL_SPAWN = 174;
	public static final int SOUND_RANDOM_ANVIL_USE = 175;
	public static final int SOUND_BOTTLE_DRAGONBREATH = 176;
	public static final int SOUND_PORTAL_TRAVEL = 177;
	public static final int SOUND_DEFAULT = 178;
	public static final int SOUND_UNDEFINED = 179;
	
	static {
		v12ToNewBook = new HashMap<>();
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ITEM_USE_ON, SOUND_ITEM_USE_ON);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_HIT, SOUND_HIT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_STEP, SOUND_STEP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FLY, SOUND_FLY);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_JUMP, SOUND_JUMP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BREAK, SOUND_BREAK);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PLACE, SOUND_PLACE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_HEAVY_STEP, SOUND_HEAVY_STEP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_GALLOP, SOUND_GALLOP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FALL, SOUND_FALL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_AMBIENT, SOUND_AMBIENT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_AMBIENT_BABY, SOUND_AMBIENT_BABY);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_AMBIENT_IN_WATER, SOUND_AMBIENT_IN_WATER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BREATHE, SOUND_BREATHE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DEATH, SOUND_DEATH);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DEATH_IN_WATER, SOUND_DEATH_IN_WATER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DEATH_TO_ZOMBIE, SOUND_DEATH_TO_ZOMBIE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_HURT, SOUND_HURT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_HURT_IN_WATER, SOUND_HURT_IN_WATER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_MAD, SOUND_MAD);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BOOST, SOUND_BOOST);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BOW, SOUND_BOW);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SQUISH_BIG, SOUND_SQUISH_BIG);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SQUISH_SMALL, SOUND_SQUISH_SMALL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FALL_BIG, SOUND_FALL_BIG);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FALL_SMALL, SOUND_FALL_SMALL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SPLASH, SOUND_SPLASH);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FIZZ, SOUND_FIZZ);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FLAP, SOUND_FLAP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SWIM, SOUND_SWIM);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DRINK, SOUND_DRINK);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_EAT, SOUND_EAT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_TAKEOFF, SOUND_TAKEOFF);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SHAKE, SOUND_SHAKE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PLOP, SOUND_PLOP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_LAND, SOUND_LAND);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SADDLE, SOUND_SADDLE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ARMOR, SOUND_ARMOR);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_MOB_ARMOR_STAND_PLACE, SOUND_MOB_ARMOR_STAND_PLACE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ADD_CHEST, SOUND_ADD_CHEST);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_THROW, SOUND_THROW);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ATTACK, SOUND_ATTACK);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ATTACK_NODAMAGE, SOUND_ATTACK_NODAMAGE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ATTACK_STRONG, SOUND_ATTACK_STRONG);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_WARN, SOUND_WARN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SHEAR, SOUND_SHEAR);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_MILK, SOUND_MILK);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_THUNDER, SOUND_THUNDER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_EXPLODE, SOUND_EXPLODE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FIRE, SOUND_FIRE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IGNITE, SOUND_IGNITE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FUSE, SOUND_FUSE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_STARE, SOUND_STARE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SPAWN, SOUND_SPAWN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SHOOT, SOUND_SHOOT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BREAK_BLOCK, SOUND_BREAK_BLOCK);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_LAUNCH, SOUND_LAUNCH);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BLAST, SOUND_BLAST);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_LARGE_BLAST, SOUND_LARGE_BLAST);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_TWINKLE, SOUND_TWINKLE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_REMEDY, SOUND_REMEDY);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_UNFECT, SOUND_UNFECT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_LEVELUP, SOUND_LEVELUP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BOW_HIT, SOUND_BOW_HIT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BULLET_HIT, SOUND_BULLET_HIT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_EXTINGUISH_FIRE, SOUND_EXTINGUISH_FIRE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ITEM_FIZZ, SOUND_ITEM_FIZZ);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_CHEST_OPEN, SOUND_CHEST_OPEN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_CHEST_CLOSED, SOUND_CHEST_CLOSED);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SHULKERBOX_OPEN, SOUND_SHULKERBOX_OPEN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SHULKERBOX_CLOSED, SOUND_SHULKERBOX_CLOSED);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ENDERCHEST_OPEN, SOUND_ENDERCHEST_OPEN);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ENDERCHEST_CLOSED, SOUND_ENDERCHEST_CLOSED);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_POWER_ON, SOUND_POWER_ON);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_POWER_OFF, SOUND_POWER_OFF);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ATTACH, SOUND_ATTACH);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DETACH, SOUND_DETACH);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DENY, SOUND_DENY);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_TRIPOD, SOUND_TRIPOD);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_POP, SOUND_POP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DROP_SLOT, SOUND_DROP_SLOT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_NOTE, SOUND_NOTE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_THORNS, SOUND_THORNS);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PISTON_IN, SOUND_PISTON_IN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PISTON_OUT, SOUND_PISTON_OUT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PORTAL, SOUND_PORTAL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_WATER, SOUND_WATER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_LAVA_POP, SOUND_LAVA_POP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_LAVA, SOUND_LAVA);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BURP, SOUND_BURP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BUCKET_FILL_WATER, SOUND_BUCKET_FILL_WATER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BUCKET_FILL_LAVA, SOUND_BUCKET_FILL_LAVA);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BUCKET_EMPTY_WATER, SOUND_BUCKET_EMPTY_WATER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BUCKET_EMPTY_LAVA, SOUND_BUCKET_EMPTY_LAVA);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ARMOR_EQUIP_CHAIN, SOUND_ARMOR_EQUIP_CHAIN);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ARMOR_EQUIP_DIAMOND, SOUND_ARMOR_EQUIP_DIAMOND);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ARMOR_EQUIP_GENERIC, SOUND_ARMOR_EQUIP_GENERIC);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ARMOR_EQUIP_GOLD, SOUND_ARMOR_EQUIP_GOLD);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ARMOR_EQUIP_IRON, SOUND_ARMOR_EQUIP_IRON);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ARMOR_EQUIP_LEATHER, SOUND_ARMOR_EQUIP_LEATHER);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_ARMOR_EQUIP_ELYTRA, SOUND_ARMOR_EQUIP_ELYTRA);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_13, SOUND_RECORD_13);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_CAT, SOUND_RECORD_CAT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_BLOCKS, SOUND_RECORD_BLOCKS);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_CHIRP, SOUND_RECORD_CHIRP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_FAR, SOUND_RECORD_FAR);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_MALL, SOUND_RECORD_MALL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_MELLOHI, SOUND_RECORD_MELLOHI);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_STAL, SOUND_RECORD_STAL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_STRAD, SOUND_RECORD_STRAD);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_WARD, SOUND_RECORD_WARD);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_11, SOUND_RECORD_11);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RECORD_WAIT, SOUND_RECORD_WAIT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_GUARDIAN_FLOP, SOUND_GUARDIAN_FLOP);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_ELDERGUARDIAN_CURSE, SOUND_ELDERGUARDIAN_CURSE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_MOB_WARNING, SOUND_MOB_WARNING);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_MOB_WARNING_BABY, SOUND_MOB_WARNING_BABY);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_TELEPORT, SOUND_TELEPORT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SHULKER_OPEN, SOUND_SHULKER_OPEN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_SHULKER_CLOSE, SOUND_SHULKER_CLOSE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_HAGGLE, SOUND_HAGGLE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_HAGGLE_YES, SOUND_HAGGLE_YES);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_HAGGLE_NO, SOUND_HAGGLE_NO);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_HAGGLE_IDLE, SOUND_HAGGLE_IDLE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_CHORUSGROW, SOUND_CHORUSGROW);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_CHORUSDEATH, SOUND_CHORUSDEATH);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_GLASS, SOUND_GLASS);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_POTION_BREWED, SOUND_POTION_BREWED);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_CAST_SPELL, SOUND_CAST_SPELL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PREPARE_ATTACK, SOUND_PREPARE_ATTACK);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PREPARE_SUMMON, SOUND_PREPARE_SUMMON);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PREPARE_WOLOLO, SOUND_PREPARE_WOLOLO);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_FANG, SOUND_FANG);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_CHARGE, SOUND_CHARGE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_CAMERA_TAKE_PICTURE, SOUND_CAMERA_TAKE_PICTURE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_LEASHKNOT_PLACE, SOUND_LEASHKNOT_PLACE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_LEASHKNOT_BREAK, SOUND_LEASHKNOT_BREAK);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_GROWL, SOUND_GROWL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_WHINE, SOUND_WHINE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PANT, SOUND_PANT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PURR, SOUND_PURR);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_PURREOW, SOUND_PURREOW);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DEATH_MIN_VOLUME, SOUND_DEATH_MIN_VOLUME);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DEATH_MID_VOLUME, SOUND_DEATH_MID_VOLUME);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_BLAZE, SOUND_IMITATE_BLAZE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_CAVE_SPIDER, SOUND_IMITATE_CAVE_SPIDER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_CREEPER, SOUND_IMITATE_CREEPER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_ELDER_GUARDIAN, SOUND_IMITATE_ELDER_GUARDIAN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_ENDER_DRAGON, SOUND_IMITATE_ENDER_DRAGON);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_ENDERMAN, SOUND_IMITATE_ENDERMAN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_EVOCATION_ILLAGER, SOUND_IMITATE_EVOCATION_ILLAGER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_GHAST, SOUND_IMITATE_GHAST);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_HUSK, SOUND_IMITATE_HUSK);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_ILLUSION_ILLAGER, SOUND_IMITATE_ILLUSION_ILLAGER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_MAGMA_CUBE, SOUND_IMITATE_MAGMA_CUBE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_POLAR_BEAR, SOUND_IMITATE_POLAR_BEAR);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_SHULKER, SOUND_IMITATE_SHULKER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_SILVERFISH, SOUND_IMITATE_SILVERFISH);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_SKELETON, SOUND_IMITATE_SKELETON);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_SLIME, SOUND_IMITATE_SLIME);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_SPIDER, SOUND_IMITATE_SPIDER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_STRAY, SOUND_IMITATE_STRAY);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_VEX, SOUND_IMITATE_VEX);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_VINDICATION_ILLAGER, SOUND_IMITATE_VINDICATION_ILLAGER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_WITCH, SOUND_IMITATE_WITCH);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_WITHER, SOUND_IMITATE_WITHER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_WITHER_SKELETON, SOUND_IMITATE_WITHER_SKELETON);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_WOLF, SOUND_IMITATE_WOLF);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_ZOMBIE, SOUND_IMITATE_ZOMBIE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_ZOMBIE_PIGMAN, SOUND_IMITATE_ZOMBIE_PIGMAN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_IMITATE_ZOMBIE_VILLAGER, SOUND_IMITATE_ZOMBIE_VILLAGER);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BLOCK_END_PORTAL_FRAME_FILL, SOUND_BLOCK_END_PORTAL_FRAME_FILL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BLOCK_END_PORTAL_SPAWN, SOUND_BLOCK_END_PORTAL_SPAWN);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_RANDOM_ANVIL_USE, SOUND_RANDOM_ANVIL_USE);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_BOTTLE_DRAGONBREATH, SOUND_BOTTLE_DRAGONBREATH);
		//v12ToV14Book.put(LevelSoundEventPacket.SOUND_PORTAL_TRAVEL, SOUND_PORTAL_TRAVEL);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_DEFAULT, SOUND_DEFAULT);
		v12ToNewBook.put(LevelSoundEventPacket.SOUND_UNDEFINED, SOUND_UNDEFINED);
	}
	
	public static Integer translateTo14Id(int v12Id) {
		return v12ToNewBook.getOrDefault(v12Id, 0);
	}

	public static int translateTo14ExtraData(int v14Id, int extraData) {
		switch (v14Id) {
			case SOUND_PLACE:
				return GlobalBlockPalette.getOrCreateRuntimeId(extraData, 0);
			default:
				return extraData;
		}
	}

	public static int translateTo16ExtraData(int v14Id, int extraData, AbstractProtocol protocol, boolean netease) {
		switch (v14Id) {
			case SOUND_PLACE:
				return AdvancedGlobalBlockPalette.getOrCreateRuntimeId(protocol, netease, extraData, 0);
			default:
				return extraData;
		}
	}

    public static int translateTo18ExtraData(int v14Id, int extraData, int pitch, AbstractProtocol protocol, boolean netease) {
        switch (v14Id) {
            case SOUND_PLACE:
                return AdvancedGlobalBlockPalette.getOrCreateRuntimeId(protocol, netease, extraData, 0);
            case SOUND_NOTE:
                return (extraData << 8) | (pitch & 0b11111111);
            default:
                return extraData;
        }
    }
}
