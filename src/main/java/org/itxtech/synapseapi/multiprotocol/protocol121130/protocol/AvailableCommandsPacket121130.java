...
public class AvailableCommandsPacket121130 extends Packet121130 {
    ...
    public void encode() {
        ...
        hardcodeEnum(enums, enumValues, HardcodeEnum.R_VALUE_PARAM);
        hardcodeEnum(enums, enumValues, HardcodeEnum.VALUE_PARAM);
        hardcodeEnum(enums, enumValues, HardcodeEnum.INT_PARAM);
        hardcodeEnum(enums, enumValues, HardcodeEnum.DIGITS_PARAM);
    ...
    private static void hardcodeEnum(List<CommandEnum> enums, List<String> enumValues, CommandEnum commandEnum) {
        enums.add(new CommandEnum(commandEnum.getName(), commandEnum.getValues()));
        enumValues.addAll(commandEnum.getValues().keySet());
    }

    private static class HardcodeEnum {
        static CommandEnum R_VALUE_PARAM = hardcodeEnum("RValueParam", "x", "y", "z");
        static CommandEnum VALUE_PARAM = hardcodeEnum("ValueParam", "dx", "dy", "dz");
        static CommandEnum INT_PARAM = hardcodeEnum("IntParam", "c");
        static CommandEnum DIGITS_PARAM = hardcodeEnum("DigitsParam", "l", "lm");

        private static CommandEnum hardcodeEnum(String name, String... values) {
            Map<String, Set<CommandEnumConstraint>> enumValues = new LinkedHashMap<>();
            for (String val : values) {
                enumValues.put(val, EnumSet.noneOf(CommandEnumConstraint.class));
            }
            return new CommandEnum(name, enumValues);
        }
    }
}
